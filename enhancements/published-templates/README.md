...
For example, if an admin wants to have a RHEL 10 image in sizes small, medium,
and large, they would probably create a base ansible role that can deploy RHEL
10, and then three small stub roles that just pass pre-determined size
parameters into the primary role.

Meanwhile the Tenant Admin is not able to create templates at all, because they
don't have access to add or modify ansible roles.
